<html>
	...
	<body background = "rosecheek.jpg">
		...
		else
		{
			...
			//$x++;
			//if($x==1)
			$construct .="id LIKE '%$search_each%' OR name LIKE '%$search_each%' OR address LIKE '%$search_each%' OR age LIKE '%$search_each%' OR sex LIKE '%$search_each%'";
			//else
			//$construct .="AND name LIKE '%$search_each%'";
			}
 
			$construct ="SELECT * FROM guest WHERE $construct";
			...
			else
			{
				...
				echo "<table border='1'>";
				echo "<tr><th>ID</th><th>Name</th><th>Address</th><th>Sex</th><th>Age</th></tr>";
