	...
	?>
		...
		if($row[2] < $now)
		{
			for($a=0; $a < $rowx; $a++)
			{
				...
				{
					$updateCheckout = "UPDATE checkout set room_no=(select room_no from guest_room_usage where guest_no = '".mysql_result($guest_out, $a, 'guest_no')."') where guest_no='".mysql_result($guest_out, $a, 'guest_no')."'";
					mysql_query($updateCheckout);
			
					$updateCheckout = "UPDATE checkout set checkout_date=now() where guest_no='".mysql_result($guest_out, $a, 'guest_no')."'";
					mysql_query($updateCheckout);
		...
		echo "<h4 style='color: red;'>There is/are guest/s occupying that room!</h4>";
