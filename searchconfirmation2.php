<html>
	...
	<body background = "rosecheek.jpg">
		...
		else
		{
			...
			//$x++;
			//if($x==1)
			$construct .="id LIKE '%$search_each%' OR reserve_id LIKE '%$search_each%'OR guest LIKE '%$search_each%' OR date_stamp LIKE '%$search_each%' OR confirm_name LIKE '%$search_each%'";
			//else
			...
			else
			{
				...
				echo "<table border='1'>";
				echo "<tr><th>ID</th><th>Reserve ID</th><th>Guest</th><th>Date Stamp</th><th>Confirm Name</th></tr>";
				while($runrows = mysql_fetch_assoc($run)){
					$id = $runrows ['id'];
					$reserve_id = $runrows ['reserve_id'];
					$guest = $runrows ['guest'];
					$date_stamp = $runrows ['date_stamp'];
		...
		<a href = "payment.php"><h4> Back </h4></a>
	</body>
</html>