...
<div id="wrapper">
	...
	<fieldset style="text-align:left; margin: left; -moz-border-radius: 15px; border-radius: 15px; border:solid 2px black;" class="donation">
		<legend> Update Recollection Rates Form </legend> 
		<form name="updaterecollectionpackage" action="updaterecollectionpackage.php"> 	
		<div id="middlerecord" class="scroll" style="float:center;">
			<?php
				$id = $_REQUEST['id'];
				$db = mysql_connect('localhost','root','root');
				mysql_select_db('sacredheart');
				$query = "SELECT * FROM recollection_package where id='$id'";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<table border='0'>";
				echo "<tr><th>Package Name</th><th>Type</th><th>Amount</th><th>Status</th></tr>";
				for($i=0; $i < $rows; $i++){
					echo "<td><input type='text' name='name' id='name' value='";
					echo mysql_result($r, $i, 'service_name');
					echo "'></td><td><input type='text' name='type' id='type' readonly value='";
					echo mysql_result($r, $i, 'guest_type');
					echo "'></td><td><input type='text' name='amount' id='amount' value='";
					echo mysql_result($r, $i, 'amount');
					echo "'></td><td><select name = 'status' id = 'status'><option value = ''></option><option value = 'Available'>Available</option><option value = 'Unavailable'>Unavailable</option></select>";
					echo "</td><td>";
