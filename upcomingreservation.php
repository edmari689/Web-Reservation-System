		...
		<br/>
		 <img src="upcoming.png">
			...
			$db = mysql_connect('localhost','root','root');
				...
				$query = "SELECT * FROM reservation INNER JOIN recollection_package ON reservation.recollection_id=recollection_package.id INNER JOIN confirmation ON reservation.id=confirmation.reserve_id where client_id='$id' and confirmation.payment_status = 'Unpaid' and reservation.status = 'Confirmed' and checkin_date > now()";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<div STYLE=' height: 200px; width: 900px; font-size: 12px; overflow: auto;'>";
				echo "<table border='1' align = 'center'>";
				echo "<tr><th>Check-In Date</th><th>Check-Out Date</th><th># of Guest</th><th>Type</th><th>Recollection Package</th><th>Status</th><th>Payment Status</th></tr>";
				for($i=0; $i < $rows; $i++){
					echo "<tr><td><p>";
			...
			$db = mysql_connect('localhost','root','root');
				mysql_select_db('sacredheart');
				$query = "SELECT * FROM reservation INNER JOIN retreat_package ON reservation.retreat_id=retreat_package.id INNER JOIN confirmation ON reservation.id=confirmation.reserve_id where client_id='$id' and confirmation.payment_status = 'Unpaid' and reservation.status = 'Confirmed' and checkin_date > now()";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<div STYLE=' height: 300px; width: 900px; font-size: 12px; overflow: auto;'>";
				echo "<table border='1' align = 'center'>";
				echo "<tr><th>Check-In Date</th><th>Check-Out Date</th><th># of Guest</th><th>Type</th><th>Retreat Package</th><th>Status</th><th>Payment Status</th></tr>";
				for($i=0; $i < $rows; $i++){
					echo "<tr><td><p>";
					echo mysql_result($r, $i, 'checkin_date');
					echo "</p></td><td><p>";
