...
<body>
		...
		<table align = 'center'>
		<tr>
		<td><h2>Recollection Expected Amount Details</h2></td>
		</tr>
		</table>
        	<?php
				$id = $_SESSION['id'];
				...
				$query = "SELECT *, (reservation.guest * recollection_package.amount) as total FROM reservation INNER JOIN confirm_client ON reservation.client_id=confirm_client.id INNER JOIN recollection_package ON reservation.recollection_id=recollection_package.id INNER JOIN confirmation ON reservation.id=confirmation.reserve_id where reservation.type='recollection' and confirmation.payment_status = 'Unpaid'and confirm_client.id='$id'";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<table border='1' align='center'>";
				echo "<tr><th>Number of Guests</th><th>Check-In Date</th><th>Check-Out Date</th><th>Package Name</th><th>Amount of Package</th><th>Total Amount</th><th>Payment Status</th></tr>";
				for($i=0; $i < $rows; $i++){
					echo "<tr><td><p>";
