...
<div id="wrapper">
	...
	<fieldset style="text-align:left; margin: left; -moz-border-radius: 15px; border-radius: 15px; border:solid 2px black;" class="donation">
		<legend> Confirmation Cancellation Form </legend> 
		<form name="deleteconfirmation" action="deleteconfirmation.php"> 	
		<div id="middlerecord" class="scroll" style="float:center;">
			<?php
				$id = $_REQUEST['id'];
				$db = mysql_connect('localhost','root','root');
				mysql_select_db('sacredheart');
				$query = "SELECT * FROM confirmation INNER JOIN reservation ON confirmation.reserve_id=reservation.id INNER JOIN confirm_client ON reservation.client_id=confirm_client.id where confirmation.id='$id'";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<table border='0'>";
				echo "<tr><th>Reservation ID</th><th>Number of Guest</th><th>Time Confirmed</th><th>Client's Name</th><th>Confirmed By</th><th>Email</th></tr>";
				for($i=0; $i < $rows; $i++){
					echo "<td><input type='text' name='reserve_id' id='reserve_id' value='";
					echo mysql_result($r, $i, 'confirmation.reserve_id');
					echo "' readonly></td><td><input type='text' name='guest' id='guest' value='";
					echo mysql_result($r, $i, 'confirmation.guest');
					echo "' readonly></td><td><input type='text' name='date_stamp' id='date_stamp' value='";
					...
					echo "' readonly></td><td><input type='text' name='confirm_name' id='confirm_name' value='";
