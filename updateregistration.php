...
<div id="wrapper">
	...
	<fieldset style="text-align:left; margin: left; -moz-border-radius: 15px; border-radius: 15px; border:solid 2px black;" class="donation">
		...
		<form name="updateregistration" action="editregistration.php"> 	
		...
		<div id="middlerecord" class="scroll" style="float:center;">
			<?php
				...
				$query = "SELECT * FROM confirm_client where id='$id'";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<table border='0'>";
				echo "<tr><th>Name</th><th>Company Name</th><th>Company Address</th><th>Contact</th><th>Sex</th><th>Email</th><th>User</th></tr>";
				for($i=0; $i < $rows; $i++){
					echo "<td><input type='text' name='name' id='name' value='";
					echo mysql_result($r, $i, 'name');
					echo "'></td><td><input type='text' name='company_name' id='company_name' value='";
					echo mysql_result($r, $i, 'company_name');
					echo "'></td><td><input type='text' name='company_address' id='company_address' value='";
					echo mysql_result($r, $i, 'company_address');
					echo "'></td><td><input type='text' name='contact' id='contact' value='";
					echo mysql_result($r, $i, 'contact');
					echo "'></td><td><select name = 'sex' id = 'sex'><option value = '";
					echo mysql_result($r, $i, 'sex');
					echo "'>";
					echo mysql_result($r, $i, 'sex');
					echo "</option><option value = 'Male'>Male</option><option value = 'Female'>Female</option></select>";
					echo "</td><td><input type='text' name='email' id='email' value='";
					echo mysql_result($r, $i, 'email');
					echo "'></td><td><input type='text' name='user' id='user' value='";
