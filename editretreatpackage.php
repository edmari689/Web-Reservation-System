...
<div id="wrapper">
	...
	<fieldset style="text-align:left; margin: left; -moz-border-radius: 15px; border-radius: 15px; border:solid 2px black;" class="donation">
		<legend> Update Retreat Rates Form </legend> 
		<form name="updateretreatpackage.php" action="updateretreatpackage.php"> 	
		<div id="middlerecord" class="scroll" style="float:center;">
			<?php
				$id = $_REQUEST['id'];
				$db = mysql_connect('localhost','root','root');
				mysql_select_db('sacredheart');
				$query = "SELECT * FROM retreat_package where id='$id'";
				$r = mysql_query($query);
				$rows = mysql_num_rows($r);
				echo "<table border='0'>";
				echo "<tr><th>Package Name</th><th>Amount</th><th>Status</th></tr>";
				for($i=0; $i < $rows; $i++){
