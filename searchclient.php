<html>
	...
	<body background = "rosecheek.jpg">
		...
		else
		{
			...
			//$x++;
			//if($x==1)
			$construct .="id LIKE '%$search_each%' OR name LIKE '%$search_each%' OR company_name LIKE '%$search_each%' OR sex LIKE '%$search_each%' OR company_address LIKE '%$search_each%'";
			//else
			//$construct .="AND name LIKE '%$search_each%'";
			}
 
			$construct ="SELECT * FROM client WHERE $construct";
			...
			else
			{
				...
				echo "<table border='1'>";
				echo "<tr><th>Client ID</th><th>Name</th><th>Company Name</th><th>Company Address</th><th>Contact</th><th>Sex</th><th>Username</th></tr>";
				while($runrows = mysql_fetch_assoc($run)){
					$id = $runrows ['id'];
					$name = $runrows ['name'];
					$company_name = $runrows ['company_name'];
					$company_address = $runrows ['company_address'];
		...
		<a href = "clients.php"><h4> Back </h4></a>
	</body>
</html>